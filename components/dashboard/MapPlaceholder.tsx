...
const createSeverityIcon = (severity: Incident['severity'], isSelected: boolean, isNew: boolean) => {
  ...
  const colorClass = colors[severity] || 'bg-gray-500';
  ...
  return L.divIcon({
    html: `<span class="marker-pulse"><span class="${colorClass} ${newClass}"></span><span class="marker-pulse-core ${colorClass} ${selectedClass}"></span></span>`,
    className: '',
    iconSize: [16, 16],
    iconAnchor: [8, 8],
  });
    ...
    useEffect(() => {
        ...
        incidents.forEach(incident => {
            const marker = L.marker([incident.lat, incident.lon], {
                icon: createSeverityIcon(incident.severity, incident.id === selectedIncidentId, newIncidentIds.has(incident.id)),
            });
